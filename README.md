...
## Contents
- [Introduction to Diffusion Maps](#Intro)
...
1. ***Construct the affinity (kernel) matrix <img src="https://latex.codecogs.com/gif.latex?K" /> 
*** 
...
*The selection of $\sigma_i$, which can be done by studying the local statistics of the neighborhood of point $x_i$, allows self-tuning of the point-to-point distances. A simple choice is $ \sigma_i=d(x_i,x_s) $, where $x_s$ is the $s$'th neighbor of point $x_i$. By experimenting, $s=7$ gives good results for all data.*
...
$$ K_{ij} = \exp(-\frac{d(x_i,x_j)^2}{4\sigma}), $$
where $\sigma$ is a manually picked constant.


2. ***Construct the normalize affinity matrix $Q$***  
Define $Q$ with entries 
$$ Q_{ij}=\frac{K_{ij}}{\sqrt{\sum_{j=1}^n K_{ij}\sum_{i=1}^n K_{ij}}}. $$
...
The right and left eigenvectors of $P$ are $$ \psi_j=e_j e_1,\quad \phi_j=e_j/e_1 $$
respectively, where $e_j$ is the eigenvector of the corresponding j-th eigenvalue of $Q$.  
Note that the multiplication and division are elementwise.


4. ***Define a diffusion map***  
Define a diffusion map $Y$ with columns $$ Y_j=[\frac{\lambda_j}{1-\lambda_j} \psi_j]. $$
...
4. ***Define a diffusion map***  
Define a diffusion map $Y$ with entries $$Y_{ij}=\frac{Z_{ij}}{\sqrt{\sum_{j=1}^{c} Z_{ij}^2}}, $$
